...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s Kawaiicoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  Kawaiicoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded Kawaiicoin transaction") + "\n" +
              "  Kawaiicoin-tx [options] -create [commands]   " + _("Create hex-encoded Kawaiicoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CKawaiicoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CKawaiicoinAddress addr(scriptPubKey);
    ...
    if (bScriptHash) {
        ...
        CKawaiicoinAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded Kawaiicoin transaction
